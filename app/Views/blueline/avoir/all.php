...
<div class="col-sm-12  col-md-12 main">  
    ...
    <div class="row tile-row">
		<div class="col-md-2 col-xs-12 tile blue"><h1><span><?=lang('application.application_avoir');?></span></h1>
		</div>
		<div class="col-md-3 col-xs-3 tile"><div class="icon-frame hidden-xs"><i class="ion-ios-bell"></i> </div><h1> <?php if(isset($avoir_due_this_month)){echo $avoir_due_this_month;} ?> <span><?=lang('application.application_avoir');?></span></h1><h2><?=lang('application.application_due_this_month');?></h2>
		</div>
		<div class="col-md-3 col-xs-3 tile"><div class="icon-frame secondary hidden-xs"><i class="ion-ios-analytics"></i> </div><h1> <?php if(isset($avoir_paid_this_month)){echo $avoir_paid_this_month;} ?> <span><?=lang('application.application_avoir');?></span></h1><h2><?=lang('application.application_paid_this_month');?></h2>
		</div>
	...
	<!-- boutons d'actions -->   
	<div class="mb-3">
		<a href="<?=base_url()?>avoir/create" class="btn btn-primary" data-toggle="mainmodal"><?=lang('application.application_create_avoir');?></a>
		<a type="button" class="btn btn-success" href="<?=base_url()?>exporter/$avoir_as_excel"><?=lang('application.application_export')?></a>
...
<div class="row">
	<div class="table-head"><?=lang('application.application_avoir');?></div>
		<div class="table-div">
			<table class="dataSorting table"  id="avoir" rel="<?=base_url()?>" cellspacing="0" cellpadding="0">
				<thead>
					<th><?=lang('application.application_pdf')?></th>
					<th hidden></th>
					<th width="70px" class="hidden-xs"><?=lang('application.application_avoir_id');?></th>
				...
				<?php foreach ($view_data['avoirs'] as $value):?>
				...
				<td class="option" width="12%">
					<div class="dropdown">
						...
						<?php 
						if ($value['status'] != config("occ_avoir_paye") 
								&& $value['status'] != config("occ_avoir_p_paye") ) { ?>
						...
						<?php } ?>
						  ...
						  <!--sent mail-->
						   <li role="presentation"><a  href="<?=base_url()?>avoir/sendfiles/<?=$value['id']?>" class="btn-option"><i class="fa fa-envelope" aria-hidden="true" title="Sent"> <?=lang('application.application_sent_to');?></i></a></li>
						</ul>
					</div>
				</td>
			</tr>
