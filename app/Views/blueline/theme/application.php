...
<head>
  ...
  <script
    src="<?= base_url('assets/blueline/js/plugins/jquery-1.12.4.min.js') ?>?ver=<?= esc($core_settings->version); ?>"></script>
  ...
  <!-- Bootstrap core CSS and JS -->
  <script
    src="<?= base_url() ?>assets/blueline/js/plugins/jquery-1.12.4.min.js?ver=<?= $core_settings->version; ?>"></script>
            ...
            <?php // if we have a submenu  
              foreach ($submenuRight as $key => $val) {
                ...
                <li id="<?= ($val->name); ?>" id="<?= strtolower($val->name); ?>" class="	<?php if ($act_uri == strtolower($val->link)) {
                  ...
                  <a <?php if ($val->actif == 1) { ?> href="<?= site_url($val->link); ?>" <?php } else { ?>
                      style="background-color: #D8DCE3;" <?php } ?>>
            ...
            <?php if ($existSubmenu == false) {
              ...
              foreach ($menu as $key => $value) { ?>
                <li id="<?= ($value->name); ?>" id="<?= strtolower($value->name); ?>" class="<?php if ($act_uri == strtolower($value->name)) {
                  ...
                  <a <?php if ($value->actif == 1) { ?> href="<?= site_url($value->link); ?>" <?php } else { ?>
                      style="background-color: #D8DCE3;" <?php } ?>>
                    <span class="menu-icon">
                    ...
                    <span class="nav-text">
                      ...
                      if ($value->name == 'messages') { ?>
                        <p class="notification-badge" id="countMessages" style="background: #ed5564;"><?= $count_messages; ?>
                        </p>
                      <?php } ?>
                    </span>
                  </a>
                  ...
                  <li>
                    ...
                    <div class="submenu">
                      <ul>
                        ...
                        <?php } ?>
                        <li><a data-toggle="mainmodal" href="<?= base_url() ?>projects/update/<?= $value->id; ?>">
                            <?= $this->lang->line('application_edit_project'); ?></li>
                        <li><a data-toggle="mainmodal" href="<?= base_url() ?>projects/tasks/<?= $value->id; ?>/add">
                            <?= $this->lang->line('application_add_task'); ?></li>
      ...
      <div class="topbar__left noselect">
        ...
        <div class="fc-dropdown shortcut-menu grid">
          <div class="grid__col-6 shortcut--item"><i class="ion-ios-paper-outline shortcut--icon"></i>
            <?= $this->lang->line('application_create_invoice'); ?></div>
          <div class="grid__col-6 shortcut--item"><i class="ion-ios-pricetags shortcut--icon"></i>
            <?= $this->lang->line('application_create_ticket'); ?></div>
          <div class="grid__col-6 shortcut--item"><i class="ion-ios-email shortcut--icon"></i>
            <?= $this->lang->line('application_write_messages'); ?></div>
        </div>
        <div>
          <i class="ion-earth topbar__icon fc-dropdown--trigger"><?php if ($notification_count > 0) { ?><span
        ...
        <?php if ($message_icon) { ?>
          <span class="hidden-xs">
            <a href="<?= site_url("messages"); ?>" title="<?= $this->lang->line('application_messages'); ?>">
              <i class="ion-archive topbar__icon"><?php if ($messages_new[0]->amount != "0") { ?><span
                    class="topbar__icon_alert"></span><?php } ?></i>
            </a>
          </span>
        <?php } ?>
        ...
        <div class="topbar noselect">
          ...
          <span class="inline visible-xs">
            <a href="<?= site_url("agent"); ?>" data-toggle="mainmodal"
              ...
              <img class="img-circle topbar-userpic" src="<?= $userimage; ?>" height="21px"> <i class="ion-chevron-down"
      ...
      <div class="topbar noselect" id="id01">
        <span class="inline visible-xs">
          <button type="button" class="btn" data-toggle="modal" data-target="#myModal"
            style="background-color:#fff;padding: 6px 6px; "> <i class="fa fa-question-circle" aria-hidden="true"
              style="    padding-right: 9px;font-size: 17px;color:#84878a;"></i></button>
          <!-- Modal -->
        ...
        <span class="hidden-xs topbar__name fc-dropdown--trigger">
          <i class="fa fa-question-circle" aria-hidden="true"
            style="    padding-right: 9px;font-size: 17px;"></i>Support<i class="ion-chevron-down"
            style="padding-left: 2px;"></i>
        </span>
          ...
          <ul>
            <li>
              <a target="blank" href="https://3click-solutions.com/guide-utilisateur-vision.pdf" data-toggle="">
                <span class="icon-wrapper"><i class="fa fa-download" aria-hidden="true"
                    style="    font-size: 11px; padding-right:10px;"></i></span>Télécharger la documentation
