...
<?php $document = $view_data['estimates']['document'];
 $department = $view_data['estimates']['department'];
?><div class="col-sm-12  col-md-12 main">

	<!-- statistiques sur les devis -->
	<div class="row tile-row">
		<div class="col-md-2 col-xs-12 tile blue">
			<h1><span><?=lang('application.application_estimate');?></span></h1>
		</div>
		...
		<div class="col-md-3 col-xs-3 tile">
			...
			<a href="<?=base_url()?>estimates/filter/invoiced"><h1> <?php if(isset($estimates_factured_this_month)){echo $estimates_factured_this_month;} ?><span> <?= lang('application.application_estimate');?></span></h1>
			<h2><?=lang('application.application_factured_month');?></h2>
			</a>
		</div>
		...
		<div class="col-md-3 col-xs-3 tile">
			...
			<a href="<?=base_url()?>estimates/filter/declined">
				<h1> <?php if(isset($estimates_refused_this_month)){echo $estimates_refused_this_month;} ?><span> <?=lang('application.application_estimate');?></span></h1>
				<h2><?=lang('application.application_refused_month');?></h2>
			</a>
		</div>
		...
		<div class="col-md-3 col-xs-3 tile">
			...
			<h1><?php if(isset($estimates_accepted_this_month)){echo $estimates_accepted_this_month;} ?> <span> <?=lang('application.application_estimate');?></span></h1>
			<h2><?=lang('application.estimates_accepted_this_month');?></h2>
			</a>
		</div>
	</div>
	<!-- boutons d'actions -->
		<div class="d-md-flex justify-content-between align-items-center">
				...
				<div class=" d-md-flex  justify-content-between">
						<div class="form-group col-md-5">
							...
							<option > </option>
								...
								<option value="mms" <?= $view_data['estimates']['department'] == 'mms' ? 'selected' : '' ?>>MMS</option>
								<option value="bim2d" <?= $view_data['estimates']['department'] == 'bim2d' ? 'selected' : '' ?>>BIM 2D</option>
								<option value="bim3d" <?= $view_data['estimates']['department'] == 'bim3d' ? 'selected' : '' ?>>BIM 3D</option>
							</select>
						</div>
						<div class=" form-group col-md-5">
							...
							<option > </option>
								...
								<option value="devis"  <?= $view_data['estimates']['document'] == 'devis' ? 'selected' : 'devis' ?>>Devis</option>
								<option value="attachement"  <?= $view_data['estimates']['document'] == 'attachement' ? 'selected' : 'attachement' ?>>Attachement</option>
							</select>
							</div>
						</div>
				</div>
			...
			<a href="<?= base_url() ?>projects" class="btn btn-primary ">Liste des projets </a>
			</div>
		...
		<a type="button" class="btn btn-success" href="<?=base_url()?>exporter/devis_as_excel"><?=lang('application.application_export')?></a>
		<!--<a href="<?=base_url()?>invoices" class="btn btn-primary"><?=lang('application.application_invoices');?></a> -->
		<!--<a href="<?=base_url()?>boncommande" class="btn btn-primary"><?=lang('application.application_commande');?></a>
		<a href="#" class="btn btn-primary"><?=lang('application.application_livraison');?></a>-->
		...
		<div class="table-div">
			<table class="dataSorting table" id="estimates" rel="<?=base_url()?>" cellspacing="0" cellpadding="0">
				<thead>
					...
					<th  width="10%"><?=lang('application.application_estimate_id');?></th>
					<th width="10%">Réference projet</th>
					<th width="10%"><?=lang('application.application_client');?></th>
					<th width="10%"><?=lang('application.application_asset');?></th>
					<th width="10%"><?=lang('application.application_issue_date');?></th>
					<th  width="10%"><?=lang('application.application_total_ttc');?></th>
					<th width="5%">Etat</th>
					<th width="1%"><?=lang('application.application_action');?></th>
					<th width="0%"></th>
				</thead>
				<?php
				$chiffre='';
				foreach ($view_data['estimates'] as $value):
					...
					<tr id="<?=$value['id'];?>" >
						...
						<td class="option" width="10%">
							<!-- bouton poubelle -->
							<a type="button" class="btn-option delete po" data-bs-toggle="popover" data-placement="left" data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?=base_url()?>estimates/delete/<?=$value['id'];?>'>
								<?=lang('application.application_yes_im_sure');?></a> <button class='btn po-close'><?=lang('application.application_no');?></button> <input type='hidden' name='td-id' class='id' value='<?=$value['id'];?>'>" data-original-title="<b><?=lang('application.application_really_delete');?>
	...
	<td class="option" width="10%">
						...
						<td width="10%" class="hidden-xs">
							<span><?php $unix = human_to_unix($value['issue_date'].' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; echo date($view_data['core_settings']['date_format'], $unix);?></span>
						</td>
						<!-- Total TTC -->
						<?php 
						...
						?>
						<td width="10%" class="hidden-xs"><?= ($company['tva'] == 1) ? number_format($ht, $value['currency']['id'], '.', ' ') : number_format($ttc, $value['currency']['id'], '.', ' ') ?></td>
						<!-- Etat -->
							...
							<div class="dropdown">
								...
								<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
								  <li role="presentation"><a href="<?=base_url()?>estimates/update/<?=$value['id'];?>" class="btn-option" data-toggle="mainmodal"><i class="fa fa-edit" title="Modifier"> Modifier</i></a></li>
									...
									<!--sent mail-->
									<li role="presentation"><a  href="<?=base_url()?>estimates/sendfiles/<?=$value['id']?>" class="btn-option"><i class="fa fa-envelope" aria-hidden="true" title="Sent"> <?=lang('application.application_sent_to');?></i></a></li>						
								</ul>
							</div>
						</td>
					</tr>
