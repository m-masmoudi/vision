...
class InvoicesController extends BaseController
{
	...
	protected $invoice, $projectModel, $invoiceHasItemModel, $expenseModel,$config;
	...
	function __construct()
	{
		...
		$this->idTypeEtatFacture = $this->config->type_id_etat_facture;
		$this->idTypeEtatAvoir = $this->config->type_id_etat_avoir;
		$this->idFactureOuvert = $this->config->occ_facture_ouvert;
		$this->idAvoirOuvert = $this->config->occ_avoir_ouvert;
		$this->idMoyensPaiement = $this->config->type_id_moyens_paiement;


		$submenus = $this->referentiels->getReferentielsByIdType($this->idTypeEtatFacture);
		$this->view_data['submenu'][lang('application.application_all')] = 'invoices';

		foreach ($submenus as $submenu) {
			if ($submenu['name'] != "Sent" && $submenu['name']!= "Pending") {
				if (lang('application.application_' . $submenu['name']) == false) {
					$this->view_data['submenu'][$submenu['name']] = 'invoices/filter' . $submenu['name'];
				} else {
					$this->view_data['submenu'][lang('application.application_' . $submenu['name'])] = 'invoices/filter' . $submenu['name'];
				}
			}
		}
	}
	...
	function index()
	{
		// var_dump($this->view_data['comp_array']);
		// die;
    ...
    ->findAll();
		...
		foreach ($facture as $key => $fact) {
			...
			$facture[$key]['chiffre'] = $this->referentiels->getReferentielsByIdType($refTypeCurrency)[0]['name'] ?? null;
		}
		...
		$this->view_data['invoices_paid_this_month_graph'] = $this->invoice->select([
			"COUNT(id) AS amount",
		...
		return view('blueline/invoices/all', ["view_data"=>$this->view_data]);
	}
	...
	public function filter($condition = false, $year = false)
	{
		...
		$this->view_data['settings'] = $this->settingModel->first();
		return view('invoices/all', $this->view_data);
	}
	...
	function create()
	{
		...
		$this->view_data['invoices'] = $this->invoice->findAll();
		$this->view_data['next_reference'] = $this->invoice->orderBy('id', 'desc')->first();
		...
		return view('invoices/_invoice', $this->view_data);
	}
	...
	public function update($id = null, $getView = false)
	{
		...
		$this->view_data['companies'] = $this->companyModel->where('inactive', '0')->findAll();
		$this->view_data['title'] = 'Edit Invoice';

		return view('invoices/edit', $this->view_data);
	}
	...
	public function view($id = null)
	{
		...
		if ($invoice['project_id'] != 0) {
			$this->view_data['project'] = $this->projectModel->find($invoice['project_id']);
		}

		// Fetch currency details
		$currencyId = $this->refType->getRefTypeByName($invoice['currency'])['id'];

		
		$ref_type=$this->referentiels->where('id_type', $currencyId)->where('visible', 1)
		->orderBy('id', 'desc')
		->get()
		->getResultArray();

		$this->view_data['chiffre'] =$ref_type[0]['name'];
		
		// Fetch company details
		$this->view_data['company'] = $this->companyModel->find($invoice['company_id']);
		$invoicehasmodel=new InvoiceHasItemModel();
		$this->view_data['items']= $invoicehasmodel->where('invoice_id',$invoice['id'])->get()->getResultArray();
		// Calculate totals
		//$this->calculateInvoiceTotals($invoice);

		return view('blueline/invoices/view', ['view_data'=>$this->view_data]);
	}
	...
	public function bankTransfer($id = null)
	{
		$invoice = $this->invoice->find($id);
		$this->view_data['invoice'] = $invoice;
		$this->view_data['title'] = 'Bank Transfer';

		return view('invoices/bank_transfer', $this->view_data);
	}
	...
	private function preparePaymentView($id)
	{
		...
		$this->view_data['payment_reference'] = $this->getPaymentReferenceCount($id) + 1;

		// Moyens de paiement
		$this->view_data['typepaiement'] = $this->referentiels->getReferentielsByIdType($this->idMoyensPaiement);
		$this->view_data['sumRest'] = $this->calculateRemainingAmount($company, $facture);

		$this->theme_view = 'modal';
		$this->view_data['title'] = lang('application.application_add_payment');
		$this->view_data['form_action'] = 'invoices/payment';
		$this->view_data['chiffre'] = $this->referentiels->getReferentielsByIdType($this->refType->getRefTypeByName($facture->currency)->id)[0]->name;
	...
	{
		if (!$payment) {
			$this->session->set_flashdata('message', 'error:' . lang('application.messages_delete_payment_error'));
		} else {
			$this->session->set_flashdata('message', 'success:' . lang('application.messages_delete_payment_success'));
		}
	}
	...
	private function handleSuccessfulPayment($charge, $invoice, $sum)
	{
		...
		$this->session->set_flashdata('message', 'success:' . lang('application.messages_payment_complete'));
	...
	{
		$this->view_data['invoices'] = $this->invoice->find_by_id($id);
		...
		$this->theme_view = 'modal';
		$this->view_data['title'] = lang('application.application_pay_with_credit_card');
	...
	private function generateAndEmailPDF($data, $parse_data, $core_settings)
	{
		...
		$filename = lang('application.application_invoice') . '_' . $core_settings->invoice_prefix . $data["invoice"]->reference;
	...
	private function sendEmailWithAttachment($data, $parse_data, $filename)
	{
		...
		if ($this->email->send()) {
			$this->session->set_flashdata('message', 'success:' . lang('application.messages_send_invoice_success'));
			...
			$this->session->set_flashdata('message', 'error:' . lang('application.messages_send_invoice_error'));
	...
	{
		return lang('application.application_invoice') . '_' . $data["core_settings"]->invoice_prefix . $data["invoice"]->reference;
	}
	...
	private function handleItemSubmission($id)
	{
		...
		if (!$item) {
			$this->session->set_flashdata('message', 'error:' . lang('application.messages_add_item_error'));
		} else {
			$this->session->set_flashdata('message', 'success:' . lang('application.messages_add_item_success'));
		}
		redirect('invoices/view/' . $_POST['facture_id']);
	}
	...
	private function prepareItemForm($id)
	{
		...
		$this->view_data['type'] = $this->db->query('SELECT libelle, id FROM items_has_family WHERE inactive=0 UNION SELECT libelle, id FROM items_has_family_parent WHERE inactive=0')->result();
		$this->view_data['tva'] = $this->db->query('SELECT * FROM ref_type_occurences WHERE id_type=9 AND visible=1')->result();
		$this->view_data['defaultTva'] = $this->settingModel->find(['id_vcompanies' => $_SESSION['current_company']])->tax;
		$this->view_data['invoice'] = $facture;
		$this->view_data['items'] = Item::find('all', ['conditions' => ['inactive=?', '0']]);
		$this->view_data['rebill'] = $this->expenseModel->find('all', ['conditions' => ['project_id=? AND (rebill=? OR invoice_id=?)', $this->view_data['invoice']->project_id, 1, $id]]);
		$this->theme_view = 'modal';
		$this->view_data['title'] = lang('application.application_add_item');
	...
	function duplicateItemEmpty($id)
	{
		...
		if (!$item) {
			$this->session->set_flashdata('message', 'error:' . lang('application.messages_duplicate_item_error'));
		} else {
			$this->session->set_flashdata('message', 'success:' . lang('application.messages_duplicate_item_success'));
		}
	...
	function item_update($id = FALSE)
	{
		...
		if ($_POST) {
			...
			if (!$item) {
				$this->session->set_flashdata('message', 'error:' . lang('application.messages_save_item_error'));
			} else {
				$this->session->set_flashdata('message', 'success:' . lang('application.messages_save_item_success'));
			}
			redirect('invoices/view/' . $_POST['facture_id']);
			...
			$this->view_data['type'] = $type->result();
			$this->theme_view = 'modal';
			$this->view_data['title'] = lang('application.application_edit_item');
	...
	function item_delete($id, $invoice_id)
	{
		...
		if (!$item) {
			$this->session->set_flashdata('message', 'error:' . lang('application.messages_delete_item_error'));
		} else {
			$this->session->set_flashdata('message', 'success:' . lang('application.messages_delete_item_success'));
		}
		redirect('invoices/view/' . $invoice_id);
	}
