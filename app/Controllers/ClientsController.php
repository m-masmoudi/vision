...
{
	protected $invoice;
	protected $company, $clientModel,$moduleModel;
...
{
    $this->view_data['companies'] = $this->company->getActiveClients();

    foreach ($this->view_data['companies'] as $key => $value) {
        // Check if client_id is set and not null before fetching client details
        if (!empty($value['client_id'])) {
            $contact_principale = $this->company->getClientById($value['client_id']);
            
            // Only set if $contact_principale is not null
            if ($contact_principale !== null) {
                $this->view_data['companies'][$key]['client_id'] = $contact_principale;
            }
        }
    }

    return view('blueline/clients/all', ['view_data' => $this->view_data]);
}
	...
	public function create($company_id = false)
{
    ...
    if ($this->request->getMethod() === 'post') {
        ...
        // Redirect to the client view page
    ...
    $this->view_data['modules'] = $this->moduleModel->where('type', 'client')->orderBy('sort', 'asc')->findAll();
    ...
    // Return the view
    return view('blueline/clients/_clients', $this->view_data);
}
	...
	public function view($id = false)
	{
		...
		// var_dump($this->view_data['contact_principale']);
		// die();

		$this->view_data['estimates'] = $this->invoice->getInvoices($id);


			// var_dump($this->view_data['estimates']);
			// die();
		$this->view_data['invoices'] = $this->invoice->getByCompany($id);

		return view('blueline/clients/view',['view_data' => $this->view_data]);
	}
	...
	public function ClientPassager()
	{
		// Get only companies where inactive is 0
		$this->view_data['companies'] = $this->company->where('inactive', 0)->findAll();
	
		// Load the view with the necessary data
		return view('blueline/clients/allPassager', ['view_data' => $this->view_data]);
	}
