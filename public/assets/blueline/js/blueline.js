...
function modalfunc() {
  document.querySelectorAll('[data-toggle="mainmodal"]').forEach((element) => {
    element.addEventListener("click", function (e) {
      console.log("here in blueline");
      NProgress.start();
      e.preventDefault();
      ...
      if (url.startsWith("#")) {
        modalElement.classList.add("show",  "clients");
      ...
      modalElement.querySelector('[data-dismiss="modal"]')?.addEventListener("click", () => {
        modalElement.classList.remove("show",  "clients");
...
$(document).on("click", ".add-row-ajax", function () {
  ...
  $("#dummyTR")
    ...
    .removeClass("hidden")
    .attr("id", "addedfield");
  $("#addedfield .user_id").html(
    $(".input-fields .user_id option:selected").text()
  );
  ...
  $.ajax({
    ...
    success: function (data) {
      console.log(data);
      $("#addedfield .option_button").attr(
        "href",
        $(".input-fields .delete_link").html() + data
      );
      $("#addedfield").attr("id", "");
    },
  });
...
$(document).on("submit", "form.quick-add-task", function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("ul li#task_dummy #dummy-href2").data(
        "link",
        baseurl + "check/" + data
      );
      $("ul li#task_dummy #dummy-href3").attr(
        "href",
        baseurl + "update/" + data
      );
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        if (reload) {
          $.get(document.URL, function (data) {
            ...
            var value = active
              .text()
              .replace('<i class="fa fa-spinner fa-spin"></i> ', "");
            active.html(value);
...
$(document).on("click", ".note-form #send", function (e) {
  var button = this;
  var content = $('textarea[name="note"]').html(
    $("#textfield").summernote("code")
  );
  var url = $(this).closest("form").attr("action");
...
$(".chosen-select").chosen({
  ...
  no_results_text: "Pas de résultat trouvé",
});
...
$(document).on("click", ".message-reply-button", function (e) {
  ...
  $(".message-content-reply")
    .slideDown("slow")
    .velocity({ opacity: 1 }, { queue: false, duration: "slow" });
});
...
function sorting_list(baseurl) {
  $(".sortable-list").sortable({
    ...
    receive: function (event, ui) {
      ...
      href2 =
        baseurl +
        "projects/move_task_to_milestone/" +
        taskId +
        "/" +
        milestoneId;
      ...
      if (ui.sender.context.childElementCount == 0) {
        $("#" + ui.sender.context.id).html(
          '<li class="notask list-item ui-state-disabled">No tasks yet</li>'
        );
  ...
  $(".sortable-list2").sortable({
    ...
    update: function (event, ui) {
      ...
      list3 = $(this).attr("id");
      href3 =
        baseurl + "projects/sort_milestone_list/" + formData3 + "/" + list3;
...
$(".priority-selector--group span").on("click", function (e, params) {
  ...
  $(".priority-selector--group span:nth-child(1)").velocity(
    { right: "0px" },
    { queue: false, easing: "easeOutCubic", duration: 200 }
  );
      ...
      this.on("success", function (file) {
        ...
        $(".data-media tbody").prepend(
          '<tr id="' +
            ...
            file.name +
            '</td><td class="hidden-xs"></td><td class="hidden-xs"><span class="label label-info tt" title="" data-original-title="Download Counter">0</span></td><td class="option " width="10%"><button type="button" class="btn-option btn-xs" ><i class="fa fa-times"></i></button><a href="http://192.168.1.121/fc-dev/projects/media/12/update/' +
            file.xhr.responseText +
            '" class="btn-option" data-toggle="mainmodal"><i class="fa fa-cog"></i></a></td></tr>'
        );
      });
    },
  };
